...
The final report is in [`Team Cold-Start - Final Report.pdf`](Team%20Cold-Start%20-%20Final%20Report.pdf) and the poster in [`Final Poster - Team 11.pdf`](Final%20Poster%20-%20Team%2011.pdf).
...
|-- .gitignore
|-- Final Poster - Team 11.pdf          # the capstone poster; its figure is regenerated by notebooks/report_figures.ipynb
|-- README.md
|-- Team Cold-Start - Final Report.pdf  # the capstone report; Figs 2-4 are regenerated by notebooks/report_figures.ipynb
`-- requirements.txt
```

 ## Data access statement

This project uses the **Amazon Reviews 2023** dataset ("Books" and "Movies and TV"
categories, including the 5-core rating files and item metadata), collected and
published by the McAuley Lab at UC San Diego (Hou et al., 2024) and available at
https://amazon-reviews-2023.github.io. The underlying reviews and product metadata
belong to Amazon and its users; the McAuley Lab distributes them for academic
research use, and this project uses them only for that purpose. Users of this
repository should review the terms of use on the dataset website before downloading.

We do not redistribute the raw dataset. Anyone can obtain it directly from the
link above and reproduce our preprocessing with `notebooks/data_filtering.ipynb`.
For convenience, the filtered subset produced by that notebook (four parquet files,
~2.67M Books interactions and 246,687 items) is shared via the Google Drive links in
**Data and outputs** below; these are derived files that remain subject to the
original dataset's terms. All model outputs and evaluation results in `outputs/`
were generated by this project and are committed to the repository.

Citation: Hou, Y., Li, J., He, Z., Yan, A., Chen, X., & McAuley, J. (2024). _Bridging language and items for retrieval and recommendation_ (Version 1). arXiv. https://arxiv.org/abs/2403.03952v1
