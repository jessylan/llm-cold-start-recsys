...
4. Run `notebooks/hyperparameter_tuning.ipynb`. Skippable — `outputs/hyperparameters.json` is committed.
5. Run `notebooks/baseline_steel_thread_cf.ipynb` It reads `outputs/hyperparameters.json`; if that file is missing it falls back to untuned `lambda=1`.
...
|-- outputs/
|   `-- hyperparameters.json            # Tuned lambda for CBHCF model - output of `hyperparameter_tuning.ipynb`
|-- recsys/
...
| `outputs/hyperparameters.json` | Tuned lambda for CBHCF model | Generated by step 4 |

Raw and filtered data are excluded from Git because they are large. Raw and filtered 
data are excluded from Git because they are large. The fast path in **Quick start** 
downloads the filtered parquet files directly; the raw files are only needed to re-run
`data_filtering.ipynb` yourself.
...
These are files used during the initial design of the project to establish extreme baseline cases
and validate the steel thread architecture. 
